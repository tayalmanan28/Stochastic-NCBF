...
def calc_safe_u(x_domain, h_domain, d_h_domain, d2_h_domain, f_x, g_x,sigma,gamma):
    ...
    d_h_domain = d_h_domain.reshape((-1, 1, 2))
    d2_h_domain = d2_h_domain.reshape((-1, 2, 2))
    f_x = f_x.reshape((-1, 2, 1)).to(d_h_domain.device.type)
    g_x = g_x.reshape((-1, 2, 1)).to(d_h_domain.device.type)
