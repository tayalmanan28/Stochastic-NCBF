...
class TestSafeMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sigma = torch.randn((2, 1))
        self.gamma = torch.randn(1)
        self.eta = torch.tensor([-0.003])

    def test_calc_safe_u(self):
        # Test case for the calc_safe_u function
        u, l = calc_safe_u(self.x_domain, self.h_domain, self.d_h_domain, self.d2_h_domain, self.f_x, self.g_x, self.sigma, self.gamma, self.eta)
